...
class TestHyperVUtilsV2(base.BaseTestCase):
    ...
    def test_create_default_reject_all_rules(self):
        (m_port, m_acl) = self._setup_security_rule_test()
        m_acl.Action = self._utils._ACL_ACTION_DENY
        self._utils._bind_security_rule = mock.MagicMock()
        self._utils._remove_virt_feature = mock.MagicMock()
        self._utils.create_default_reject_all_rules(self._FAKE_PORT_NAME)

        self.assertEqual(1, len(self._utils._remove_virt_feature.mock_calls))
        self.assertEqual(8, len(self._utils._bind_security_rule.mock_calls))

    def test_bind_security_rule(self):
        (m_port, m_acl) = self._setup_security_rule_test()
