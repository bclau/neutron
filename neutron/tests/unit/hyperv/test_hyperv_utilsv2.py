...
class TestHyperVUtilsV2R2(base.BaseTestCase):
    ...
    def test_get_new_deny_weight_deny(self):
        mockacl = self._create_acl(self._utils._ACL_ACTION_DENY, 0)
        self.assertEqual(1, self._utils._get_new_deny_weight([mockacl]))

    def test_get_new_deny_weight_deny(self):
        mockacl = self._create_acl(self._utils._ACL_ACTION_ALLOW, 0)
        self.assertEqual(0, self._utils._get_new_deny_weight([mockacl]))

    def _create_acl(self, action, weight):
        mockacl = mock.MagicMock()
        mockacl.Action = action
        mockacl.Weight = weight
        return mockacl
