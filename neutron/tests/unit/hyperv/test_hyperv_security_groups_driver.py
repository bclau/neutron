...
class TestHyperVSecurityGroupsDriver(base.BaseTestCase):
    ...
    def test_update_port_filter_new_port(self):
        mock_port = self._get_port()
    ...
    def test_create_port_rules_exception(self):
        ...
        self._driver._utils.create_security_rule = mock.MagicMock(
            side_effect=Exception('Generated Exception for testing.'))
    ...
    def test_create_port_rules(self):
        fake_rule = self._create_security_rule()
        self._driver._create_param_map = mock.MagicMock(
            return_value={self._FAKE_PARAM_NAME: self._FAKE_PARAM_VALUE})
        self._driver._create_port_rules(self._FAKE_ID, [fake_rule])
