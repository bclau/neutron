...
class HyperVUtilsV2(utils.HyperVUtils):
    ...
    def connect_vnic_to_vswitch(self, vswitch_name, switch_port_name):
        ...
        self._enable_port_ceilometer(switch_port_name)
    ...
    def _add_virt_feature(self, vm, res_setting_data):
        vs_man_svc = self._conn.Msvm_VirtualSystemManagementService()[0]
    ...
    def _enable_port_ceilometer(self, switch_port_name):
        ...
        acl_ins = [v for v in acls if
                   v.Action == 3 and v.Direction == self._METRIC_DIR_IN]
        acl_outs = [v for v in acls if
                    v.Action == 3 and v.Direction == self._METRIC_DIR_OUT]

        # add the ACLs, if necessary.
        if not acl_ins:
            acl_in = self._get_acl_setting_data(self._METRIC_DIR_IN)
            self._add_virt_feature(port, acl_in)
        if not acl_outs:
            acl_out = self._get_acl_setting_data(2)
            self._add_virt_feature(port, acl_out)

    def _get_acl_setting_data(self, direction):
        acl = self._get_default_setting_data(self._PORT_ALLOC_ACL_SET_DATA)
        acl.AclType = self._METRIC_ACL_TYPE
        acl.Action = self._METRIC_ACTION
        acl.Applicability = self._METRIC_APPLIC
