...
class HyperVSecurityGroupsDriver(firewall.FirewallDriver):
    ...
    def __init__(self):
        ...
        if utilsfactory.get_sec_geoup_gen():
            self._sg_gen = SecurityGroupRuleGeneratorR2()
        else:
            self._sg_gen = SecurityGroupRuleGenerator()
    ...
    def prepare_port_filter(self, port):
        ...
        if port['device'] not in self._security_ports:
            LOG.info('Creating default reject rules.')
            self._sec_group_rules[port['id']] = []
            def_sg_rules = self._sg_gen.create_default_security_group_rules()
            LOG.info("Default rules: (%s): %s" % (len(def_sg_rules),
                [s.to_dict() for s in def_sg_rules]))
    ...
    def remove_port_filter(self, port):
        ...
        self._sec_group_rules.pop(port['port_id'], None)
...
class SecurityGroupRuleGenerator(object):
    ...
    def create_security_group_rule(self, rule):
        #TODO: return [] instead, if rule contains ports?
    ...
    def create_default_security_group_rules(self):
        ...
        for direction in _ACL_PROP_MAP['direction'].values():
            for acl_type, address in ip_type_pairs:
                sg_rules.append(SecurityGroupRuleR2(acl_type=acl_type,
                                                    direction = direction,
                                                    remote_addr=address,
                                                    action=action))
        return sg_rules
...
class SecurityGroupRule(SecurityGroupRuleBase):
    ...
    _FIELDS = ["AclType", "Action", "Direction", "RemoteAddress",
...
class SecurityGroupRuleR2(SecurityGroupRuleBase):
    ...
    def __init__(self, direction, local_port, protocol, remote_addr,
        ...
        self.Direction = direction
        self.Action = action
        self.LocalPort = str(local_port)
        self.Protocol = protocol
        self.RemoteIPAddress = remote_addr
        self.Stateful = (protocol is not _ACL_PROP_MAP['protocol']['icmp'] and
