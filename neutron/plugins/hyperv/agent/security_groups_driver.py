...
class HyperVSecurityGroupsDriver(firewall.FirewallDriver):
    ...
    def _create_port_rules(self, port_id, rules):
        for rule in rules:
            ...
            try:
                ...
                LOG.error(_('Hyper-V Exception: %s while adding rule: %s'),
                          ex, rule)
    ...
    def _remove_port_rules(self, port_id, rules):
        for rule in rules:
            ...
            try:
                ...
                LOG.error(_('Hyper-V Exception: %s while removing rule: %s'),
                          ex, rule)
    ...
    def _create_param_map(self, rule):
        if 'port_range_min' in rule and 'port_range_max' in rule:
            ...
            local_port = self._get_acl_default()

        return {
            'direction': self._get_acl_dir(rule),
            'acl_type': self._get_acl_type(rule),
    ...
    def filter_defer_apply_on(self):
        LOG.info('Filter defer applying on')

    def filter_defer_apply_off(self):
        LOG.info('Filter defer applying off')

    @property
    def ports(self):
        return self._security_ports

    def _get_acl_dir(self, rule):
        return self._ACL_PROP_MAP['direction'][rule['direction']]

    def _get_acl_type(self, rule):
        return self._ACL_PROP_MAP['ethertype'][rule['ethertype']]
